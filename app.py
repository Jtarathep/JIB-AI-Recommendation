...
# Theme is controlled by a small, always-visible switch in the top-right.
# Using a URL parameter avoids Streamlit's default button styling from making
# the switch unreadable when the app itself is in the light theme.
query_theme = str(st.query_params.get("theme", "light")).lower()
if query_theme not in {"light", "dark"}:
    query_theme = "light"
if "ui_theme" not in st.session_state or st.session_state.ui_theme != query_theme:
    st.session_state.ui_theme = query_theme

ui_theme = st.session_state.ui_theme
next_theme = "dark" if ui_theme == "light" else "light"
theme_label = "🌙  โหมดมืด" if ui_theme == "light" else "☀️  โหมดสว่าง"
theme_hint = "เปลี่ยนเป็นธีมมืด" if ui_theme == "light" else "เปลี่ยนเป็นธีมสว่าง"
...
st.markdown(
    f'<div class="kk-theme-marker kk-theme-{ui_theme}"></div>'
    f'<div class="kk-theme-switch-wrap"><a class="kk-theme-switch kk-theme-switch-{ui_theme}" href="?theme={next_theme}" title="{theme_hint}">{theme_label}</a></div>',
...
/* Theme switch: always visible at the top-right of the main content.
   It is a real link, not a Streamlit button, so it keeps the same appearance
   in both light and dark themes. */
.kk-theme-switch-wrap {display:flex;justify-content:flex-end;align-items:center;margin:0 0 .65rem;min-height:46px;}
.kk-theme-switch {display:inline-flex;align-items:center;justify-content:center;gap:.45rem;min-height:44px;padding:.55rem 1.05rem;border-radius:999px;text-decoration:none !important;font-size:.84rem;font-weight:850;letter-spacing:.01em;transition:transform .16s ease,box-shadow .16s ease,background .16s ease,border-color .16s ease;}
.kk-theme-switch:hover {transform:translateY(-1px);text-decoration:none !important;}
.kk-theme-switch-light {color:#173052 !important;background:linear-gradient(135deg,#ffffff,#eaf1fb);border:1px solid #b9cbe4;box-shadow:0 8px 22px rgba(45,66,105,.12),inset 0 1px 0 #fff;}
.kk-theme-switch-light:hover {color:#102743 !important;background:#ffffff;box-shadow:0 10px 26px rgba(45,66,105,.16);}
.kk-theme-switch-dark {color:#f7f9ff !important;background:linear-gradient(135deg,#18294a,#232053);border:1px solid #667eea;box-shadow:0 8px 24px rgba(43,65,150,.28),0 0 20px rgba(90,76,255,.12);}
.kk-theme-switch-dark:hover {color:#fff !important;background:linear-gradient(135deg,#21365f,#2d2869);}
...
body:has(.kk-theme-light) .stApp{background:#f5f7fb;}
body:has(.kk-theme-light) [data-testid="stAppViewContainer"],body:has(.kk-theme-light) [data-testid="stMainBlockContainer"]{background:#f5f7fb;}
body:has(.kk-theme-light) [data-testid="stSidebar"]{background:linear-gradient(180deg,#f8fbff 0%,#eef3fa 100%);border-right:1px solid #d7e0ed;}
body:has(.kk-theme-light) [data-testid="stSidebar"] *{color:#263954;}
body:has(.kk-theme-light) .kk-title{background:linear-gradient(90deg,#10234a 0%,#1766b8 48%,#6547c7 100%);-webkit-background-clip:text;background-clip:text;color:transparent;}
body:has(.kk-theme-light) .kk-subtitle{color:#40516d;}
body:has(.kk-theme-light) .kk-subtitle strong{color:#182844;}
body:has(.kk-theme-light) .kk-badge{color:#15233c;background:linear-gradient(145deg,#ffffff,#edf3ff);border-color:#c9d7ef;box-shadow:0 8px 22px rgba(47,72,120,.10),inset 0 1px 0 #fff;}
body:has(.kk-theme-light) .kk-badge span{color:#5c6d87;}
body:has(.kk-theme-light) .kk-step{color:#53637b;background:linear-gradient(145deg,#f8fbff,#edf3fb);border-color:#d3dff0;}
body:has(.kk-theme-light) .kk-step strong{color:#172640;}
body:has(.kk-theme-light) .kk-flow{background:#f5f8fd;border-color:#d7e1ef;}
body:has(.kk-theme-light) .kk-note{color:#52627b;background:linear-gradient(90deg,#effaf8,#f2f4ff);border-color:#b8dcd6;}
body:has(.kk-theme-light) .kk-note b{color:#172640;}
body:has(.kk-theme-light) .kk-edit-heading,body:has(.kk-theme-light) .kk-candidate-title,body:has(.kk-theme-light) .kk-preview-title b{color:#182844;}
body:has(.kk-theme-light) .kk-edit-sub,body:has(.kk-theme-light) .kk-preview-title small{color:#66758d;}
body:has(.kk-theme-light) .kk-preview-title{background:linear-gradient(135deg,#f4f7ff,#ffffff);border-color:#d3deef;}
body:has(.kk-theme-light) .kk-side-brand .brand{color:#172640;}
body:has(.kk-theme-light) .kk-side-card{background:linear-gradient(145deg,#ffffff,#f1f5fb);border-color:#d6e0ee;color:#263954;}
body:has(.kk-theme-light) .kk-side-label{color:#70809a;}
body:has(.kk-theme-light) [data-testid="stChatMessage"]{background:linear-gradient(145deg,#ffffff,#f3f7fd);border-color:#cfdaeb;box-shadow:0 10px 28px rgba(42,63,100,.09),inset 0 1px 0 #fff;}
body:has(.kk-theme-light) [data-testid="stChatMessage"] [data-testid="stMarkdownContainer"]{color:#1d2b42;}
body:has(.kk-theme-light) .kk-user-bubble{color:#13223d;background:linear-gradient(135deg,#e7e9ff,#e4f0ff);border-color:#b8c8ec;box-shadow:0 8px 24px rgba(66,87,150,.10),inset 0 1px 0 #fff;}
body:has(.kk-theme-light) [data-testid="stChatInput"]{background:linear-gradient(135deg,#ffffff,#f0f4fb) !important;border-color:#9fb5df !important;box-shadow:0 0 0 1px rgba(63,102,190,.06),0 10px 28px rgba(45,65,105,.12) !important;}
body:has(.kk-theme-light) [data-testid="stChatInput"] textarea{color:#172640 !important;}
body:has(.kk-theme-light) [data-testid="stChatInput"] textarea::placeholder{color:#7586a0 !important;}
body:has(.kk-theme-light) .stButton>button{color:#1c2c47 !important;background:linear-gradient(145deg,#ffffff,#eef3fa) !important;border-color:#c4d1e5 !important;box-shadow:inset 0 1px 0 #fff,0 4px 14px rgba(48,68,105,.06) !important;}
body:has(.kk-theme-light) .stButton>button:hover{background:linear-gradient(145deg,#f7faff,#e8effa) !important;border-color:#6f94d4 !important;box-shadow:0 8px 22px rgba(56,91,158,.12) !important;}
body:has(.kk-theme-light) [data-testid="stExpander"]{background:#ffffff !important;border-color:#d5dfed !important;}
body:has(.kk-theme-light) [data-testid="stExpander"] summary{color:#20324f !important;}
body:has(.kk-theme-light) [data-testid="stAlert"]{background:#f5f8fd !important;color:#263954 !important;border-color:#d2ddec !important;}
body:has(.kk-theme-light) [data-testid="stAlert"] p,body:has(.kk-theme-light) [data-testid="stAlert"] div{color:#263954 !important;}
body:has(.kk-theme-light) .stMarkdown,body:has(.kk-theme-light) .stCaption,body:has(.kk-theme-light) label,body:has(.kk-theme-light) [data-testid="stMetricLabel"],body:has(.kk-theme-light) [data-testid="stMetricValue"]{color:#243650;}
body:has(.kk-theme-light) a{color:#2469c8 !important;}
body:has(.kk-theme-light) h1,body:has(.kk-theme-light) h2,body:has(.kk-theme-light) h3,body:has(.kk-theme-light) h4,body:has(.kk-theme-light) h5,body:has(.kk-theme-light) h6{color:#172640;}
body:has(.kk-theme-light) [data-testid="stTextInput"] input,body:has(.kk-theme-light) textarea,body:has(.kk-theme-light) select{color:#172640 !important;}
/* Strong, readable light theme */
body:has(.kk-theme-light) {background:#f4f7fb !important;}
body:has(.kk-theme-light) .stApp,body:has(.kk-theme-light) [data-testid="stAppViewContainer"],body:has(.kk-theme-light) [data-testid="stMainBlockContainer"] {background:#f4f7fb !important;}
body:has(.kk-theme-light) .kk-hero {background:radial-gradient(circle at 86% 18%,rgba(119,72,255,.13),transparent 28%),radial-gradient(circle at 70% 92%,rgba(0,170,255,.10),transparent 30%),linear-gradient(135deg,#eef7ff 0%,#f3f2ff 55%,#f8f5ff 100%);border-color:#c8d8f0;box-shadow:0 18px 48px rgba(49,72,112,.12),inset 0 1px 0 #fff;}
body:has(.kk-theme-light) .kk-eyebrow {color:#1768c7;}
body:has(.kk-theme-light) .kk-title {background:linear-gradient(90deg,#102c61 0%,#1766b8 48%,#6547c7 100%);-webkit-background-clip:text;background-clip:text;color:transparent;}
body:has(.kk-theme-light) .kk-subtitle {color:#304664;} body:has(.kk-theme-light) .kk-subtitle strong {color:#102b55;}
body:has(.kk-theme-light) .kk-badge {background:linear-gradient(145deg,#ffffff,#edf4ff);color:#152844;border-color:#c9d8ee;box-shadow:inset 0 1px 0 #fff,0 8px 24px rgba(54,78,120,.08);} body:has(.kk-theme-light) .kk-badge span {color:#536782;}
body:has(.kk-theme-light) .kk-flow {background:rgba(255,255,255,.78);border-color:#cfdceb;box-shadow:0 8px 24px rgba(54,78,120,.06);} body:has(.kk-theme-light) .kk-step {background:linear-gradient(145deg,#ffffff,#f0f5fc);color:#52637c;border-color:#cfdbeb;} body:has(.kk-theme-light) .kk-step strong {color:#152844;}
body:has(.kk-theme-light) .kk-note {color:#344a68;background:linear-gradient(90deg,#effaf8,#f2f5ff);border-color:#b9d8d5;} body:has(.kk-theme-light) .kk-note b {color:#122947;}
body:has(.kk-theme-light) [data-testid="stChatMessage"] {background:linear-gradient(145deg,#ffffff,#f2f6fc);border-color:#cbd9ea;box-shadow:0 10px 28px rgba(42,63,100,.08),inset 0 1px 0 #fff;}
body:has(.kk-theme-light) [data-testid="stChatMessage"] [data-testid="stMarkdownContainer"],body:has(.kk-theme-light) [data-testid="stChatMessage"] p,body:has(.kk-theme-light) [data-testid="stChatMessage"] li,body:has(.kk-theme-light) [data-testid="stChatMessage"] strong {color:#172942 !important;}
body:has(.kk-theme-light) .kk-user-bubble {color:#122542 !important;background:linear-gradient(135deg,#e7edff,#e9f5ff);border-color:#b9cbea;box-shadow:0 8px 24px rgba(66,87,150,.09),inset 0 1px 0 #fff;}
body:has(.kk-theme-light) [data-testid="stChatInput"] {background:linear-gradient(135deg,#ffffff,#eef4fc) !important;border-color:#9fb8df !important;box-shadow:0 8px 26px rgba(45,65,105,.12) !important;} body:has(.kk-theme-light) [data-testid="stChatInput"] textarea {color:#122542 !important;-webkit-text-fill-color:#122542 !important;} body:has(.kk-theme-light) [data-testid="stChatInput"] textarea::placeholder {color:#6d809d !important;}
body:has(.kk-theme-light) [data-testid="stBottomBlockContainer"],body:has(.kk-theme-light) [data-testid="stBottom"],body:has(.kk-theme-light) section[data-testid="stChatInput"],body:has(.kk-theme-light) .stChatFloatingInputContainer,body:has(.kk-theme-light) [data-testid="stBottomBlockContainer"] > div,body:has(.kk-theme-light) [data-testid="stBottom"] > div,body:has(.kk-theme-light) section[data-testid="stChatInput"] > div,body:has(.kk-theme-light) .stChatFloatingInputContainer > div,body:has(.kk-theme-light) [data-testid="stChatInput"] > div {background:#f4f7fb !important;}
body:has(.kk-theme-light) [data-testid="stChatInput"] [data-baseweb="textarea"],body:has(.kk-theme-light) [data-testid="stChatInput"] [data-baseweb="base-input"],body:has(.kk-theme-light) [data-testid="stChatInput"] [data-testid="stChatInputTextArea"] {background:transparent !important;color:#122542 !important;}
body:has(.kk-theme-light) [data-testid="stChatInput"] button {background:linear-gradient(135deg,#6b4cff,#7b4dff) !important;color:#fff !important;border-color:#a99cff !important;box-shadow:0 5px 16px rgba(90,70,230,.22) !important;}
body:has(.kk-theme-light) .stButton > button {color:#162943 !important;background:linear-gradient(145deg,#ffffff,#eef4fb) !important;border-color:#bfd0e5 !important;box-shadow:inset 0 1px 0 #fff,0 4px 14px rgba(48,68,105,.06) !important;} body:has(.kk-theme-light) .stButton > button:hover {color:#102542 !important;background:linear-gradient(145deg,#ffffff,#e7f0fb) !important;border-color:#6d94d4 !important;}
body:has(.kk-theme-light) [data-testid="stMarkdownContainer"],body:has(.kk-theme-light) .stMarkdown,body:has(.kk-theme-light) .stCaption,body:has(.kk-theme-light) label,body:has(.kk-theme-light) [data-testid="stMetricLabel"],body:has(.kk-theme-light) [data-testid="stMetricValue"] {color:#203653 !important;}
body:has(.kk-theme-light) [data-testid="stMarkdownContainer"] p,body:has(.kk-theme-light) [data-testid="stMarkdownContainer"] li,body:has(.kk-theme-light) [data-testid="stMarkdownContainer"] span,body:has(.kk-theme-light) [data-testid="stMarkdownContainer"] div {color:#203653;}
body:has(.kk-theme-light) h1,body:has(.kk-theme-light) h2,body:has(.kk-theme-light) h3,body:has(.kk-theme-light) h4,body:has(.kk-theme-light) h5,body:has(.kk-theme-light) h6 {color:#142742 !important;} body:has(.kk-theme-light) a {color:#1767c5 !important;}
body:has(.kk-theme-light) [data-testid="stAlert"],body:has(.kk-theme-light) [data-testid="stAlert"] p,body:has(.kk-theme-light) [data-testid="stAlert"] div {color:#203653 !important;}
body:has(.kk-theme-light) [data-testid="stExpander"] {background:#ffffff !important;border-color:#d1ddeb !important;color:#203653 !important;} body:has(.kk-theme-light) [data-testid="stExpander"] summary {color:#203653 !important;}
body:has(.kk-theme-light) .kk-edit-heading,body:has(.kk-theme-light) .kk-candidate-title,body:has(.kk-theme-light) .kk-preview-title b {color:#142742 !important;} body:has(.kk-theme-light) .kk-edit-sub,body:has(.kk-theme-light) .kk-preview-title small {color:#60738f !important;} body:has(.kk-theme-light) .kk-preview-title {background:linear-gradient(135deg,#f4f7ff,#ffffff);border-color:#cbd9ea;}
body:has(.kk-theme-light) .kk-side-card {background:linear-gradient(145deg,#ffffff,#f0f5fb);border-color:#d3dfed;color:#263954;} body:has(.kk-theme-light) .kk-side-brand .brand {color:#172640;} body:has(.kk-theme-light) .kk-side-label {color:#6c7d97;}
</style>
