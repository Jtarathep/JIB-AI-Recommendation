...
st.markdown(f'<div class="kk-theme-marker kk-theme-{ui_theme}"></div>', unsafe_allow_html=True)

theme_left, theme_right = st.columns([7.2, 2.0], gap="small")
with theme_right:
    theme_label_top = "☀️  เปลี่ยนเป็นโหมดสว่าง" if st.session_state.ui_theme == "dark" else "🌙  เปลี่ยนเป็นโหมดมืด"
    if st.button(theme_label_top, use_container_width=True, key="theme_toggle_top"):
        toggle_theme()
        st.rerun()
...
.kk-theme-topbar {display:flex;justify-content:flex-end;align-items:center;margin:0 0 .65rem;min-height:44px;}
.kk-theme-topbar .stButton {margin-left:auto;}
.kk-theme-topbar .stButton > button {min-height:42px !important;padding:.45rem 1rem !important;border-radius:999px !important;font-weight:800 !important;letter-spacing:.01em;}
.kk-theme-topbar .stButton > button:hover {transform:translateY(-1px) scale(1.01) !important;}
